...

{
  ...
  delete inFile;
  edm::LogError("EcalEBPhase2TPParamProducer") << " Values for binOfMaximum and/or nSamplesToUse are wrong. The "
                                                  "default  binOfMaximum=6 and  nSamplesToUse=8 will be used";
}
...
void EcalEBPhase2TPParamProducer::analyze(const edm::Event& evt, const edm::EventSetup& evtSetup) {
  ...
  for (const auto& it : ebCells) {
    ...
    for (unsigned int i = 0; i < ecalPh2::NGAINS; ++i) {
      ok = computeLinearizerParam(theta, ecalph2::gains[i], calibCoeff, shift, mult);
      if (!ok) {
